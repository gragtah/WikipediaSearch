...
f = open(filename, 'r')
term_to_file_position_start = int(f.readlines()[-1])
term_to_file_position_end = f.tell()
f.seek(term_to_file_position_start, 0)
term_to_file_position = marshal.loads(f.read(term_to_file_position_start - term_to_file_position_end))

index = QuerySideIndex(f, term_to_file_position)
k_gram = None

# Read total number of documents in collection.
if not os.path.exists(sys.argv[3]):
    print ""
    print "Title Index : " + sys.argv[3] + " does not exist."
    print ""
    sys.exit()
...
def ranked_results(setOfDocIDs, term_to_idf):
...
def print_ranked_results(setOfDocIDs, term_to_idf):
    print_docIDs(ranked_results(setOfDocIDs, term_to_idf))
...
def processFTQ(keywordList, term_to_idf):
    ...
    print_ranked_results(resultDocs, term_to_idf)
...
def processWQ(query, term_to_idf):
    global k_gram
    if k_gram == None:
        k_gram = KGramIndex(index.lookup(K_GRAM_DB_IDENTIFIER))
